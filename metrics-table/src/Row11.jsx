    ...
    <>
      <tr key={row.id + "-1"} className="row-width">
        <td className="table-cell">
          <button onClick={() => toggleDropdown11(index)} className="dropdown-button column-id">
            {row.id}
          </button>
        </td>
        <td className="table-cell column-text" colSpan={3}>{row.text}</td>
      </tr>
      <tr key={row.id + "-subrow"} className={`${dropdown11Visible === index ? 'visible' : 'hidden'} dropdown11`}>
          ...
          <table>
            <tbody className="dropdown11">
              ...
              <tr key={rowIndex}>
                ...
                <td className="table-cell column-text">{dropdownRow.text2}</td>
                <div>
                <td className="table-cell">
                  <input
                    type="text"
