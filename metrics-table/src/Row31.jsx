...
<tr key={row.id + "-1"} className="fourth-row">
  <td className="table-cell">
    <button onClick={() => toggleDropdown31(index)} className="dropdown-button-style">
      {row.id}
    </button>
  </td>
  <td className="table-cell">{row.text}</td>
</tr>
<tr key={row.id + "-subrow"} className={`table-row ${dropdown31Visible === index ? 'visible' : 'hidden'}`}>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (inputValue.startsWith(prefix)) {
                        handleDropdown31Change(rowIndex, "input1", inputValue);
                      } else {
                        handleDropdown31Change(rowIndex, "input1", prefix + inputValue);
                      }
                    }}
                    onKeyDown={(e) => {
                      const prefix = "Από μέτρηση: ";
